...
as(
	...
	, MIN(SI.rows) AS 'RowCount'
	, MIN(SIR.rows) AS 'ReferencedRowCount'
  FROM sys.foreign_key_columns FKC (NOLOCK)
  INNER JOIN sys.objects o (NOLOCK) ON FKC.parent_object_id = o.object_id
  INNER JOIN sys.sysindexes SI (NOLOCK) ON o.object_id = SI.id AND SI.indid <= 1
  INNER JOIN sys.objects RO (NOLOCK) ON FKC.referenced_object_id = RO.object_id
  INNER JOIN sys.sysindexes SIR (NOLOCK) ON RO.object_id = SIR.id AND SIR.indid <= 1
...
SELECT 
  ...
  fk.[RowCount] TableRowCount,
  fk.[ReferencedRowCount] ReferencedTableRowCount
FROM FKTable fk 
