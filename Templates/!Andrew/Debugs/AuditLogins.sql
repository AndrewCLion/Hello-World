...
AND (		(s.sid <> 1) AND (s.sysadmin = 1)		
		OR (s.sid <> 1) AND (s.securityadmin = 1)		
		OR (s.sid <> 1) AND (s.serveradmin = 1)		
		OR (s.sid <> 1) AND (s.setupadmin = 1)		
		OR (s.sid <> 1) AND (s.processadmin = 1)		
		OR (s.sid <> 1) AND (s.diskadmin = 1)		
		OR (s.sid <> 1) AND (s.dbcreator = 1)		
		OR (s.sid <> 1) AND (s.bulkadmin = 1)	
		)	
GO
