...
SELECT        p.name AS loginname, p.type, p.type_desc, p.is_disabled, CONVERT(VARCHAR(10), p.create_date, 101) AS created, CONVERT(VARCHAR(10), p.modify_date, 101) 
                         AS [update], s.sysadmin, s.securityadmin, s.serveradmin, s.setupadmin, s.processadmin, s.diskadmin, s.dbcreator, s.bulkadmin
FROM            sys.server_principals AS p INNER JOIN
                         sys.syslogins AS s ON p.sid = s.sid
WHERE        (p.type_desc IN ('SQL_LOGIN', 'WINDOWS_LOGIN', 'WINDOWS_GROUP')) AND (p.name LIKE 'BYACCOUNT\IN%BV')
GO
